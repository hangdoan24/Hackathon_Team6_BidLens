...
function renderCriteria() {
  $("#criteria-list").innerHTML = criteria.map((c, i) => `
    <div class="crit-row">
      ...
      <input type="range" min="5" max="30" value="${c.weight}" data-i="${i}" class="crit-range" />
  ...
  document.querySelectorAll(".crit-chk").forEach(el =>
    el.addEventListener("change", () => { criteria[el.dataset.i].enabled = el.checked; checkReady(); }));
  document.querySelectorAll(".crit-range").forEach(el =>
    el.addEventListener("input", () => {
      criteria[el.dataset.i].weight = +el.value;
...
function renderReadiness(sr) {
  ...
  const key = sr.status.toLowerCase();
  ...
  $("#readiness-summary").textContent = sr.summary || "";
...
function renderTraceability(reqs) {
  ...
  $("#trace-list").innerHTML = reqs.map((r, i) => {
    ...
    <div class="finding-card">
      <div class="finding-header" onclick="togglePanel('${id}')">
        <span class="trace-req-id">${esc(r.id)}</span>
        <span class="status-tag ${st}">${st}</span>
        <span class="severity-badge ${sev}">${sev}</span>
        <span class="trace-req-text">${esc(r.requirement)}</span>
        <span class="finding-chevron" id="chev-${id}">▼</span>
      </div>
      <div class="finding-body" id="body-${id}">
        ${rfpQ ? `<div class="evidence-block rfp-ev"><div class="ev-label">RFP — ${esc(r.rfp_section || "")}</div><div class="ev-quote">"${esc(rfpQ)}"</div></div>` : ""}
        ${propQ ? `<div class="evidence-block prop-ev"><div class="ev-label">Proposal — ${esc(propS || "")}</div><div class="ev-quote">"${esc(propQ)}"</div></div>` : `<div class="evidence-block" style="background:var(--coral-dim);border-left:3px solid var(--coral)"><div class="ev-label" style="color:var(--coral)">Proposal</div><div class="ev-quote" style="color:var(--text-muted)">No matching evidence found</div></div>`}
        ${r.business_impact ? `<div class="impact-line">${esc(r.business_impact)}</div>` : ""}
        ${r.suggested_fix ? `<div class="fix-line">${esc(r.suggested_fix)}</div>` : ""}
      </div>
    </div>`;
  }).join("");
}
  ...
  try {
    ...
    if (data.suggested_weights) {
      data.suggested_weights.forEach(sw => {
        ...
        if (m) m.weight = Math.max(5, Math.min(30, sw.weight));
      });
      renderCriteria();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("#rfp-sample").addEventListener("change", async (e) => {
    if (!e.target.value) return;
    try { $("#rfp-text").value = await loadSample(e.target.value); $("#rfp-count").textContent = $("#rfp-text").value.length; checkReady(); toast("Loaded", "success"); }
    catch { toast("Failed to load"); }
  });
  $("#proposal-sample").addEventListener("change", async (e) => {
    if (!e.target.value) return;
    try { $("#proposal-text").value = await loadSample(e.target.value); $("#proposal-count").textContent = $("#proposal-text").value.length; checkReady(); toast("Loaded", "success"); }
    catch { toast("Failed to load"); }
  });
